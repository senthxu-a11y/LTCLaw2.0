...
from ltclaw_gy_x.config.config import (
    AgentProfileConfig,
    AgentsRunningConfig,
    load_agent_config,
    save_agent_config,
)
from ltclaw_gy_x.constant import (
    LLM_BACKOFF_BASE,
    LLM_BACKOFF_CAP,
    LLM_MAX_RETRIES,
)
...
def mock_agent_workspace(tmp_path, monkeypatch):
    ...
    import json
    from ltclaw_gy_x.config.utils import get_config_path
    from ltclaw_gy_x.config.config import Config, AgentsConfig, AgentProfileRef

    # Setup workspace directory
    workspace_dir = tmp_path / "workspaces" / "test_agent"
    workspace_dir.mkdir(parents=True, exist_ok=True)

    monkeypatch.setattr("ltclaw_gy_x.config.utils.WORKING_DIR", tmp_path)
    monkeypatch.setattr("ltclaw_gy_x.config.config.WORKING_DIR", tmp_path)
