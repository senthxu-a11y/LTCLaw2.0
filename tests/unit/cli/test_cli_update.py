...
from ltclaw_gy_x.cli.update_cmd import (
    InstallInfo,
    RunningServiceInfo,
...
def _install_info(
    *,
    ...
    return InstallInfo(
        package_dir="/tmp/site-packages/ltclaw_gy_x",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"url": "file:///tmp/ltclaw_gy_x.whl"},
            ("local", "file:///tmp/ltclaw_gy_x.whl"),
        ),
        (
            {"url": "https://example.com/ltclaw_gy_x.whl"},
            ("direct-url", "https://example.com/ltclaw_gy_x.whl"),
        ),
    ],
)
...
def test_detect_installation(
    ...
    from ltclaw_gy_x.cli import update_cmd as update_cmd_module
...
def test_update_reports_up_to_date(monkeypatch) -> None:
    ...
    assert "LTCLAW-GY.X is already up to date." in result.output
...
def test_probe_service_ignores_proxy_env(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ltclaw_gy_x.cli.update_cmd.httpx.get", _fake_get)
...
def test_update_blocks_running_service(monkeypatch) -> None:
    ...
    assert result.exit_code != 0
    assert "Please stop it before running `ltclaw_gy_x update`" in result.output
    assert (
...
def test_update_can_cancel_forced_shutdown(monkeypatch) -> None:
    ...
    assert result.exit_code == 0
    assert (
        "forcibly terminate the current LTCLAW-GY.X backend/frontend "
...
def test_update_can_force_shutdown_running_service(
    ...
    def _fake_shutdown(
        ...
        class _Result:
            returncode = 0
            stdout = "Stopped LTCLAW-GY.X processes: 1234\n"
    ...
    assert result.exit_code == 0
    assert "Running `ltclaw_gy_x shutdown` before updating..." in result.output
    assert "Stopped LTCLAW-GY.X processes: 1234" in result.output
    assert "Starting LTCLAW-GY.X update..." in result.output
...
def test_update_worker_waits_for_launcher_exit(
    ...
    monkeypatch.setattr(
        "ltclaw_gy_x.cli.update_cmd._wait_for_process_exit",
...
def test_run_update_worker_detached_spawns_without_capture(
    ...
    monkeypatch.setattr(
        "ltclaw_gy_x.cli.update_cmd._spawn_update_worker",
        _fake_spawn,
    )
...
def test_update_worker_foreground_streams_output_and_cleans_plan(
    ...
    assert return_code == 0
    assert "[ltclaw_gy_x] Updating LTCLAW-GY.X 1.0.0 -> 9.9.9..." in captured.out
    assert "[ltclaw_gy_x] Using installer: integration-test" in captured.out
    assert "installer: preparing" in captured.out
    assert "installer: done" in captured.out
    assert "[ltclaw_gy_x] Update completed successfully." in captured.out
...
def test_update_worker_foreground_propagates_failure_exit_code(
    ...
    assert "[ltclaw_gy_x] Update failed with exit code 7." in captured.out
    assert not plan_path.exists()
