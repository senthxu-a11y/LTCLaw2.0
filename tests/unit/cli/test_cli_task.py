# -*- coding: utf-8 -*-
"""Tests for the ``ltclaw_gy_x task`` headless CLI command."""
...
from ltclaw_gy_x.cli.task_cmd import _read_instruction
...
def test_task_rejects_empty_instruction(monkeypatch) -> None:
    monkeypatch.setattr(
        "ltclaw_gy_x.config.config.load_agent_config",
        MagicMock(),
    )
...
def test_model_flag_overrides_agent_config(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "ltclaw_gy_x.cli.task_cmd._run_task",
        AsyncMock(
...
def test_output_dir_writes_result_json(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("ltclaw_gy_x.cli.task_cmd._run_task", _fake_run_task)
...
def _build_guarded_agent(request_context: dict):
    from ltclaw_gy_x.agents.tool_guard_mixin import ToolGuardMixin
...
async def test_tool_guard_not_bypassed_without_flag():
    ...
    with patch(
        "ltclaw_gy_x.security.tool_guard.engine.get_guard_engine",
    ) as mock_engine_fn:
        ...
        with patch("ltclaw_gy_x.app.approvals.get_approval_service"):
...
def test_e2e_cli_no_guard_and_skills_dir(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("ltclaw_gy_x.cli.task_cmd._run_task", _spy_run_task)
...
@pytest.mark.skipif(
    os.name == "nt", reason="symlink requires elevated privileges on Windows"
)
def test_isolated_workspace_creates_overlay(tmp_path):
    """Overlay workspace symlinks skills and pre-populates manifest."""
    from ltclaw_gy_x.cli.task_cmd import _isolated_skills_workspace
    from ltclaw_gy_x.agents.skills_manager import resolve_effective_skills
