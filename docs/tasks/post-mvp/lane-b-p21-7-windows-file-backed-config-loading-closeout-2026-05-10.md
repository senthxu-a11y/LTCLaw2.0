...
## Mac Review Follow-Up Result

The first Mac review did find two blockers, but both were in test construction rather than in production source code.

What the Mac review showed:

1. the failing `test_config.py` addition depended on directly assigning a raw dict into a typed `external_provider_config` field and then expecting post-validation serialization behavior without going through `ProjectConfig` validation
2. the failing `test_game_project_router.py` addition combined the project router and release router but only used FastAPI dependency override for `get_agent_for_request`, while the release status route reads that seam through the existing module-level async helper path
3. no production `src/` defect was required to explain either Mac failure
4. no `src/` file was changed in the Mac follow-up fix

Mac follow-up fix scope:

1. `tests/unit/game/test_config.py` was adjusted to validate through `ProjectConfig.model_validate(...)` before asserting JSON-safe dump and secret stripping
2. `tests/unit/routers/test_game_project_router.py` was adjusted to monkeypatch `release_router_module.get_agent_for_request` in the same way as the existing release-router tests
3. no production source behavior was changed

Mac executable validation result:

1. `.venv/bin/python -m pytest tests/unit/game/test_config.py tests/unit/game/test_service.py tests/unit/routers/test_game_project_router.py tests/unit/game/test_knowledge_rag_provider_selection.py -q` -> `37 passed`
2. `.venv/bin/python -m pytest tests/unit/game/test_config.py tests/unit/game/test_service.py tests/unit/routers/test_game_project_router.py tests/unit/game/test_knowledge_rag_provider_selection.py tests/unit/game/test_knowledge_rag_answer.py tests/unit/routers/test_game_knowledge_rag_router.py -q` -> `106 passed`

Mac review conclusion:

1. the temporary merge blocker was caused by test construction and test seam usage, not by a production source regression
2. Mac regression is now green for the reviewed slice
3. this remains not production ready and not production rollout
4. after the review-branch test fix, this P21.7 slice is suitable for merge review into `main`
...
6. the Mac review blockers were resolved by test-only fixes
7. Mac regression now passes for the reviewed P21.7 slice

Residual limit:

1. executable pytest confirmation could not be completed on the original Windows test machine because `pytest` is missing from that venv
...
1. merge the reviewed P21.7 test-only hardening into `main`
2. keep the P21.5 Windows fake-endpoint receipt as `passed with one restart-only caveat`
3. if needed, re-run P21.5 Windows fake-endpoint smoke after merge using the same local-only fake endpoint boundary
4. treat all follow-up validation as not production rollout