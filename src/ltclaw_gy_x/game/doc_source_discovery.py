...
from .config import DEFAULT_DOCS_EXCLUDE_PATTERNS, ProjectDocsSourceConfig


AVAILABLE_DOC_FORMATS: dict[str, str] = {
    '.md': 'md',
    '.txt': 'txt',
    '.docx': 'docx',
}

COLD_START_SUPPORTED_DOC_FORMATS: frozenset[str] = frozenset({'md', 'txt', 'docx'})
...
def _entry(source_path: str, fmt: str, status: str, reason: str) -> dict:
    cold_start_supported = fmt in COLD_START_SUPPORTED_DOC_FORMATS and status == 'available'
    cold_start_reason = (
...
def discover_document_sources(project_root: Path | None, docs_config: ProjectDocsSourceConfig | None) -> dict:
    ...
    empty_payload = {
        ...
        'next_action': 'configure_docs_source',
    }
    ...
    effective_config = docs_config or ProjectDocsSourceConfig()
    if not effective_config.roots:
        empty_payload['errors'].append({'reason': 'docs_roots_not_configured'})
        empty_payload['summary']['error_count'] = len(empty_payload['errors'])
        return empty_payload

    include_patterns = [pattern.strip() for pattern in effective_config.include if pattern.strip()]
    exclude_patterns = list(dict.fromkeys([*DEFAULT_DOCS_EXCLUDE_PATTERNS, *effective_config.exclude]))
    ...
    for configured_root in effective_config.roots:
        ...
        for file_path in root_path.rglob('*'):
            ...
            if suffix not in AVAILABLE_DOC_FORMATS:
                continue
            doc_files.append(_entry(normalized_path, AVAILABLE_DOC_FORMATS[suffix], 'available', 'matched_supported_format'))
            seen_paths.add(normalized_path)

    available_count = sum(1 for item in doc_files if item['cold_start_supported'])
    return {
        ...
        'errors': errors,
        'summary': {
            'discovered_doc_count': len(doc_files),
            'available_doc_count': available_count,
            'excluded_doc_count': len(excluded_files),
            'unsupported_doc_count': 0,
            'error_count': len(errors),
        },
        'next_action': 'run_doc_index' if available_count > 0 else 'configure_docs_source',
    }
